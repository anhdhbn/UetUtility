...
{
    public class Common<T> where T : Base
    {
        ...
        internal static void Copy(object obj, BaseEntity baseEntity)
        {
            throw new NotImplementedException();
        }

        public static void Split(ICollection<T> New, ICollection<T> Current, out List<T> Insert, out List<T> Update, out List<T> Delete, out List<T> NoChange)
        {
            Insert = new List<T>();
            Update = new List<T>();
            Delete = new List<T>();
            ...
            {
                T tmp = New.Where(N => N.EqualsProperties(C)).FirstOrDefault();
                if (tmp == null)
                    Delete.Add(C);
                else
                {
                    T tmp2 = New.Where(N => N.Equals(C)).FirstOrDefault();
                    if (tmp2 == null) Update.Add(tmp);
                    else NoChange.Add(tmp2);
                }
            }
            foreach (T N in New)
            {
                if (!Current.Any(C => C.EqualsProperties(N)))
                    Insert.Add(N);
            }
        }
    }
